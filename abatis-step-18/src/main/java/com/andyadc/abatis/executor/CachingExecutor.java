...
import com.andyadc.abatis.cache.TransactionalCacheManager;
...
public class CachingExecutor implements Executor {
    ...
    private final Logger logger = LoggerFactory.getLogger(CachingExecutor.class);
        ...
        if (cache != null) {
            ...
            if (ms.isUseCache() && resultHandler == null) {
                ...
                if (logger.isDebugEnabled() && cache.getSize() > 0) {
                    logger.debug("二级缓存：{}", Utils.toJSONString(list));
                }
                return list;
            }
        }
